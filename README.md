...
规则策略和 Python 策略默认以紧凑摘要显示，保留名称、版本状态、已保存策略选择、「编辑策略」和「删除版本」入口。载入策略后可直接运行回测；点击「编辑策略」才展开规则或代码、参数、校验和保存操作。收起编辑器保留草稿，切换编辑模式或恢复历史回测时默认重新收起。

删除时先从「载入我的策略」选中已保存版本，再点击「删除版本」，确认具体名称和版本号。仅移除该版本，其他版本、行情和历史回测快照保留；当前内容变为未保存草稿，可以另存为新策略。内置策略不支持删除。数据库采用删除标记保留版本序号，不会重新使用已删除的版本号，也不承诺释放数据库空间。删除接口为 `POST /api/strategy-editor/definitions/{definition_id}/delete`，不存在或已删除的版本返回 404。
