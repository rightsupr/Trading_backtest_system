...
默认适配尾盘交易：以当日完整日线计算信号，用当日收盘价加减滑点成交。滑点设为 0 时直接按收盘价成交，佣金和税仍单独扣除。完整收盘价、成交量等在尾盘决策时尚未完全确定，这是一种尾盘近似，可能高估可实现表现；精确模拟 14:55 等时点需要分钟或逐笔数据。

已保存的历史回测不改写；界面会标识旧次日开盘口径，点击重新运行后按新的当日收盘口径生成新记录。Python、文件、内置及旧规则策略共用撮合引擎，输出信号无需自行向前或向后移动。

| 项目 | V0.1 行为 |
|---|---|
| 方向/仓位 | 单股票、只做多、尽可能满仓买入、全部卖出 |
| 信号 | 收盘后确认，逐日输出 BUY/SELL/HOLD/NONE、position_target、reason |
| 成交 | `signal_at_close` + `execute_same_close`（默认）：当日信号、当日收盘价成交，最后一日信号也可撮合；`execute_next_open` 保留用于旧回测复现 |
| 买入 | `close × (1 + slippage)`，100 股整数倍，先预留佣金 |
| 卖出 | `close × (1 - slippage)`，扣佣金及卖出印花税 |
...
| T+1 | 尾盘买入后最早下一交易日尾盘卖出；逐日仅执行一个信号，不支持同日买卖 |
...
- MFE / MAE 使用实际持仓期间日高/低相对成交买价的最大有利/不利幅度（零为基准）；当日收盘模式排除买入日成交前的高低点，包含卖出日全天高低点；旧次日开盘模式保留原统计口径。它们是税费前价格幅度。
...
测试不请求真实外网；使用隔离的临时 DuckDB。覆盖指标参考计算、因果性、参数验证、整手买入、当日收盘撮合及旧次日开盘兼容、滑点、最低佣金、卖出税、收益/回撤、交易生成、期末持仓、停牌、资金不足、重复写入、来源/复权隔离、增量范围、复权基准变化、API 及历史快照。
